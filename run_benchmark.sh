...
[ -z ${distro} ] && SOURCE=1
LOG_FILE=${LOG_FILE:-$base_dir/benchmarks/$bmname/results}
startdir="$base_dir/benchmarks/$bmname"
...
if ! [ -f bm_installed ]; then
    ...
    if [ -z ${SOURCE} ]; then
        ...
        create_srcpackage
        cd $srcdir
        echo "$base_dir/suite/$bmname.patch"
        [ -f "$base_dir/suite/$bmname.patch" ] && echo patch && cp "$base_dir/suite/$bmname.patch" $srcdir
